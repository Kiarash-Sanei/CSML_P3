...
asm_main:
    ...
    ; Read elements of first matrix (float type)
    xor r12, r12             ; Initialize counter
    input_loop1:
        mov rdi, scanf_format_float
        lea rsi, DWORD [matrix1 + r12 * 4]    ; Calculate address of current element (float)
        xor eax, eax
        call scanf
    ...
    ; Read elements of second matrix (float type)
    xor r12, r12               ; Initialize counter
    input_loop2:
        mov rdi, scanf_format_float
        lea rsi, DWORD [matrix2 + r12 * 4]    ; Calculate address of current element (float)
        xor eax, eax
        call scanf
        inc r12                         ; Move to next element
    ...
    outer_multiplication_loop:
        ...
        middle_multiplication_loop:
            ...
            xorps xmm15, xmm15       ; Initialize sum for current element
            inner_multiplication_loop:
                ...
                imul eax, DWORD [p]
                add rax, r13
                movss xmm1, DWORD [matrix2 + rax * 4]
            ...
            imul eax, DWORD [p]
            add rax, r13
            movss DWORD [matrix3 + rax * 4], xmm15
    ...
    outer_print_loop:
        ...
        inner_print_loop:
            ; Calculate and print matrix3[i][j] (float type)
            mov rax, r12
            imul eax, DWORD [p]
            add rax, r13
            movss xmm0, [matrix3 + rax * 4]
            cvtss2sd xmm0, xmm0 ; convert float to double
            mov rdi, printf_format_float
            call printf
...
section .data
    ...
    scanf_format_float: db "%f", 0      ; Format for reading floats
    printf_format_float: db "%.2f ", 0    ; Format for printing floats
...
section .bss
    ...
    matrix1: resd 10000                ; Storage for first matrix (float)
    matrix2: resd 10000                ; Storage for second matrix (float)
    matrix3: resd 10000                ; Storage for result matrix (float)
